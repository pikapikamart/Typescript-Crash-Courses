...
export async function createUserSessionHandler(req: Request, res: Response) {
  ...
  const user = await validatePassword(req.body); // This will return the user

  if ( !user ) return res.status(401).send("Invalid email or password");

  const session = await createSession(user._id, req.get("user-agent") || ""); // This will return session from db
